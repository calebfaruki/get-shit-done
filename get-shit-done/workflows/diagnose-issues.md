...
After UAT finds gaps, spawn one debug agent per gap. Each agent investigates autonomously with symptoms pre-filled from UAT. Collect root causes, update UAT.md gaps with diagnosis, then present results to the user.
...
UAT tells us WHAT is broken (symptoms). Debug agents find WHY (root cause). Targeted fixes can then be based on actual causes, not guesses.
...
- `{goal}`: `find_root_cause_only` (UAT flow - human decides how to fix)
...
**Suggested Fix Direction:** {brief hint for resolution}
```
...
Present diagnosis results to the user.
Offer next steps: fix manually, re-run the phase, or adjust the plan.
</step>
...
Agents only diagnose — the human decides how to fix (no fix application).
</context_efficiency>
...
- Fall back to reporting gaps without root causes (less precise)
</failure_handling>
...
- [ ] Present results to user with next step options
</success_criteria>
