...
No git commits — all changes are working tree only per user decision.
  ...
  <verify>Run `npm test` — 81+ tests pass (0 failures). Run `node --test tests/deletion-safety.test.cjs` — all characterization tests pass. Verify deleted files don't exist: `ls commands/gsd/audit-milestone.md 2>&1` returns "No such file".</verify>
  <done>~38 dead files deleted across 3 waves. Full test suite passes after each wave. No regressions.</done>
</task>
